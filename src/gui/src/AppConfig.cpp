...
const char* AppConfig::m_SynergySettingsName[] = {"screenName",
                                                  "port",
                                                  "interface",
                                                  "logLevel2",
                                                  "logToFile",
...
void AppConfig::loadSettings() {
  ...
  {  // Scope related code together
    ...
    if (!elevateMode.isValid()) {
      elevateMode = loadSetting(kElevateModeSetting, QVariant(static_cast<int>(defaultElevateMode)));
    }
  ...
  m_eliteBackersUrl =
      loadCommonSetting(kEliteBackersUrl, "https://api2.prod.symless.com/credits/elite-backers").toString();
  m_guid = loadCommonSetting(kGuid, QUuid::createUuid()).toString();
  m_licenseRegistryUrl =
      loadCommonSetting(kLicenseRegistryUrl, "https://api2.prod.symless.com/license/register").toString();
  ...
  bool updateSerial = ConfigWriter::make()->hasSetting(settingName(kLoadSystemSettings), ConfigWriter::kCurrent);
  ...
  QString certificateFilename = QString("%1/%2/%3").arg(m_CoreInterface.getProfileDir(), "SSL", "Synergy.pem");
...
void AppConfig::setLastVersion(const QString& version) { setSettingModified(m_lastVersion, version); }
...
void AppConfig::setNetworkInterface(const QString& s) { setSettingModified(m_Interface, s); }
...
void AppConfig::setLogFilename(const QString& s) { setSettingModified(m_LogFilename, s); }

void AppConfig::setWizardHasRun() { setSettingModified(m_WizardLastRun, kWizardVersion); }

void AppConfig::setLanguage(const QString& language) { setSettingModified(m_Language, language); }

void AppConfig::setStartedBefore(bool b) { setSettingModified(m_StartedBefore, b); }

void AppConfig::setElevateMode(ElevateMode em) { setSettingModified(m_ElevateMode, em); }
...
void AppConfig::setLastExpiringWarningTime(int newValue) { setSettingModified(m_LastExpiringWarningTime, newValue); }
#endif
...
bool AppConfig::isCryptoAvailable() const {
  ...
  //   result = (edition() == kPro || edition() == kPro_China || edition() == kBusiness || edition() == kUltimate);
  // #endif

  return result;
}

bool AppConfig::getCryptoEnabled() const { return isCryptoAvailable() && m_CryptoEnabled; }

void AppConfig::setAutoHide(bool b) { setSettingModified(m_AutoHide, b); }

bool AppConfig::getAutoHide() { return m_AutoHide; }

void AppConfig::setMinimizeToTray(bool newValue) { setSettingModified(m_MinimizeToTray, newValue); }

bool AppConfig::getInvertScrollDirection() const { return m_InvertScrollDirection; }

void AppConfig::setEliteBackersUrl(const QString& newValue) { setSettingModified(m_eliteBackersUrl, newValue); }

void AppConfig::setLicenseNextCheck(unsigned long long time) { setSettingModified(m_licenseNextCheck, time); }

const QString& AppConfig::getEliteBackersUrl() const { return m_eliteBackersUrl; }

const QString& AppConfig::getLicenseRegistryUrl() const { return m_licenseRegistryUrl; }

unsigned long long AppConfig::getLicenseNextCheck() const { return m_licenseNextCheck; }

const QString& AppConfig::getGuid() const { return m_guid; }

bool AppConfig::getLanguageSync() const { return m_LanguageSync; }

void AppConfig::setInvertScrollDirection(bool newValue) { setSettingModified(m_InvertScrollDirection, newValue); }

void AppConfig::setLanguageSync(bool newValue) { setSettingModified(m_LanguageSync, newValue); }

bool AppConfig::getPreventSleep() const { return m_PreventSleep; }

bool AppConfig::getClientHostMode() const { return (m_ClientHostMode && getInitiateConnectionFromServer()); }

bool AppConfig::getServerClientMode() const { return (m_ServerClientMode && getInitiateConnectionFromServer()); }

bool AppConfig::getInitiateConnectionFromServer() const { return m_InitiateConnectionFromServer; }

void AppConfig::setPreventSleep(bool newValue) { setSettingModified(m_PreventSleep, newValue); }

bool AppConfig::getMinimizeToTray() { return m_MinimizeToTray; }

QString AppConfig::settingName(AppConfig::Setting name) { return m_SynergySettingsName[name]; }
...
bool AppConfig::isWritable() const { return ConfigWriter::make()->isWritable(); }

bool AppConfig::isSystemScoped() const { return ConfigWriter::make()->getScope() == ConfigWriter::kSystem; }
...
void AppConfig::setUseExternalConfig(bool newValue) { setSettingModified(m_UseExternalConfig, newValue); }

void AppConfig::setConfigFile(const QString& newValue) { setSettingModified(m_ConfigFile, newValue); }

void AppConfig::setUseInternalConfig(bool newValue) { setSettingModified(m_UseInternalConfig, newValue); }

void AppConfig::setClientGroupChecked(bool newValue) { setSettingModified(m_ClientGroupChecked, newValue); }

void AppConfig::setServerHostname(const QString& newValue) { setSettingModified(m_ServerHostname, newValue); }

void AppConfig::setClientHostMode(bool newValue) { setSettingModified(m_ClientHostMode, newValue); }

void AppConfig::setServerClientMode(bool newValue) { setSettingModified(m_ServerClientMode, newValue); }
...
void AppConfig::setTLSCertPath(const QString& path) { m_TLSCertificatePath = path; }
