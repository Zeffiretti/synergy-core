...
std::string getMaintenanceMessage(const SerialKey& serialKey) {
  ...
  QString message =
      "The license key you used will only work with versions of Synergy released before %1."
...
LicenseManager::LicenseManager(AppConfig* appConfig)
    : m_AppConfig(appConfig), m_serialKey(appConfig->edition()), m_registry(*appConfig) {}
...
void LicenseManager::notifyUpdate(QString fromVersion, QString toVersion) const {
  return;
  ...
  notifier->setUpdateInfo(fromVersion, toVersion, QString::fromStdString(m_serialKey.toString()));
...
QString LicenseManager::activeEditionName() const { return getEditionName(activeEdition(), m_serialKey.isTrial()); }
...
void LicenseManager::skipActivation() const { notifyActivation("skip:unknown"); }
